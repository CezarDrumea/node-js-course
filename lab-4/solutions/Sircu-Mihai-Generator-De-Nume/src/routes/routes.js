...
// Removed: import axios from 'axios'; // axios is no longer needed here

const SECRET = 'supersecret'; // Should be from environment variables in production
// Removed: const JSON_SERVER_URL = 'http://localhost:4000'; // JSON_SERVER_URL is no longer needed here
...
export async function requireAuth(req, res, next) {
  ...
  if (isNonEmpty(sessionId)) {
    try {
      ...
      if (session?.userId) {
        // Use authModel.getUserById instead of axios.get
